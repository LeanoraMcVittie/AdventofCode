...
class OxygenField(Field):
	...
	def max_distance_from_cell(self, start_cell: Loc) -> int:
		seen: List[Loc] = []
		this_level: List[Loc] = [start_cell]
		i = -1
		while this_level:
			i += 1
			seen.extend(this_level)
			next_level = []
			for c in this_level:
				self.apply_adjacent(
					c,
					False,
					filterer=lambda n: n.value == "." and n not in seen,
					transform=lambda n: next_level.append(n)
				)
...
def run(input_data: List[str]) -> int:
	...
	oxygen.map()
	oxygen.field.print()
	return oxygen.field.max_distance_from_cell(oxygen.oxygen_cell)
